...
function params(){ const p=new URLSearchParams(location.search); return {schoolYear:p.get('year')||String(new Date().getFullYear()),className:p.get('class')||''}; }
function blankStore(){ return {version:1,logs:[],progress:{},lastBackupAt:null}; }
function readStore(){ try{const v=JSON.parse(localStorage.getItem(STORAGE_KEY)||'null');return v&&v.version===1?v:blankStore();}catch(_){return blankStore();} }
function writeStore(store){ try{localStorage.setItem(STORAGE_KEY,JSON.stringify(store));return true;}catch(_){toast('保存できませんでした。JSONバックアップを作成してください。');return false;} }
...
function toast(msg){ const el=document.getElementById('toast');if(!el)return;el.textContent=msg;el.hidden=false;clearTimeout(toast._t);toast._t=setTimeout(()=>{el.hidden=true;},1800); }
...
function addLog(type,note=''){
  ...
  const rec={recordId:`log_${Date.now()}_${Math.random().toString(36).slice(2,8)}`,createdAt:nowIso(),date:todayLocal(),schoolYear:p.schoolYear,className:p.className,lessonId:s.lessonId||'',questionId:s.questionId||'',questionKey:s.questionKey||'',focus:s.focus||q.focus||'',type,note:String(note||'').trim()};
  store.logs.push(rec);if(!writeStore(store))return null;renderPastCount();return rec;
}
function deleteLog(id){
  const store=readStore();const rec=store.logs.find(x=>x.recordId===id);if(!rec)return;
  if(!confirm(`${typeLabel(rec.type)} の記録を削除しますか？`))return;
  store.logs=store.logs.filter(x=>x.recordId!==id);writeStore(store);showPast();renderPastCount();toast('記録を削除しました');
}
function logsForCurrent(){ const s=state(),store=readStore();return store.logs.filter(x=>x.lessonId===s.lessonId&&x.questionId===s.questionId).sort((a,b)=>String(b.createdAt).localeCompare(String(a.createdAt))); }

function saveProgress(){
  const s=state(),p=params(),store=readStore();
  if(!s.lessonId){toast('教材IDを取得できません');return false;}
  if(!p.className){toast('My Hubからクラス指定で開くと進度を保存できます');return false;}
...
function downloadBackup(){
  const store=readStore();store.lastBackupAt=nowIso();writeStore(store);
  const blob=new Blob([JSON.stringify(store,null,2)],{type:'application/json'});const url=URL.createObjectURL(blob);const a=document.createElement('a');a.href=url;a.download=`teaching-backup-${todayLocal()}-${new Date().toTimeString().slice(0,8).replaceAll(':','')}.json`;document.body.appendChild(a);a.click();a.remove();setTimeout(()=>URL.revokeObjectURL(url),1000);updateBackupLabel();
}
function endLesson(){ const saved=saveProgress();downloadBackup();toast(saved?'進度を保存し、JSONバックアップを出力しました':'JSONバックアップを出力しました'); }
...
function saveQuickNote(){ const input=document.getElementById('teacherQuickNote');const note=String(input?.value||'').trim();if(!safeNote(note))return;const id=input?.dataset.recordId;const store=readStore();const rec=store.logs.find(x=>x.recordId===id);if(rec){rec.note=note;writeStore(store);}closeQuick();renderPastCount();toast('メモを追加しました'); }
...
function injectUI(){
  document.body.classList.add('teacher-mode');const p=params(),ui=readUi();
  const dock=document.createElement('aside');dock.id='teacherDock';dock.className='teacher-dock';dock.innerHTML=`<div class="teacher-dock-card"><div class="teacher-dock-head" id="teacherDockHandle"><div class="teacher-head-main"><span>TEACHER ${escapeHtml(p.className?`· ${p.className}`:'· クラス未指定')}</span><span id="teacherBackupState"></span></div><div class="teacher-head-tools"><button id="teacherGuideBtn" class="teacher-head-btn" type="button" title="教師用の使い方">ⓘ</button><button id="teacherMinBtn" class="teacher-head-btn" type="button" title="最小化">−</button></div></div><div class="teacher-dock-body"><div class="teacher-types">${TYPES.map(([type,icon,label])=>`<button class="teacher-type" type="button" data-log-type="${type}" title="${label}">${icon} ${label}</button>`).join('')}</div><div class="teacher-quick"><input id="teacherQuickNote" hidden><button id="teacherQuickSave" class="teacher-action" type="button" hidden>保存</button><button id="teacherQuickSkip" class="teacher-action" type="button" hidden>閉じる</button></div><small id="teacherQuickHint" class="teacher-hint" hidden>個人を特定できる情報は入力しない。</small><div class="teacher-footer"><button id="teacherPastBtn" class="teacher-action" type="button">🧠 過去記録 0</button><button id="teacherEndBtn" class="teacher-action primary" type="button">授業終了</button></div></div></div>`;document.body.appendChild(dock);
...
function resumeIfRequested(){ const url=new URL(location.href);if(url.searchParams.get('resume')!=='1')return;const s=state(),p=params(),store=readStore(),key=progressKey(s.lessonId,p.schoolYear,p.className),saved=store.progress[key];if(!saved||!Number.isInteger(saved.resumeSlideIndex))return;const ok=window.LessonEngine?.jumpTo?.(saved.resumeSlideIndex);if(ok){const message=saved.completed?'このLessonは前回までに完了しています':`続き：${saved.resumeQuestionKey||saved.questionKey||'保存位置'}から`;setTimeout(()=>toast(message),50);}}
